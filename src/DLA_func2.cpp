...
// [[Rcpp::export]]
List DLA_func2(int size, int boundary) {
  //Set Seed
...
//    } // end seed while loop
    ...
    //1) Define boundary
    IntegerMatrix boundary_points(max_bound*2,2);
    for (int i=0;i<max_bound;i++) {
      //Top boundary
      boundary_points(i,0) = i;
      boundary_points(i,1) = max_bound;
      //Side boundary
      boundary_points(max_bound+i,0) = max_bound;
      boundary_points(max_bound+i,1) = i;
    }
    //2) Draw random number and sample from boundary
    int set_seed = rand() % (max_bound*2 + 1 - 1) + 1;
    //3) Define seed
    ...
    while (check == 1) {
      ...
      //Calculate distance btw the new seed and each node in the dla tree
      NumericVector euclid_dist(num_node);
      for (int i=0;i<num_node;i++) {
      ...
      // Is the random walk within the bounds?
      if ((min_dist < (boundary+10)) && (seed_new(0) >= 1) && (seed_new(1) >= 1)) {
